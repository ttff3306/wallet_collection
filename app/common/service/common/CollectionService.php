                ...
                if ($token_total_token_value > 0)
                {
                    switch ($chain)
                    {
                        ...
                        case 'TRON':
                            ...
                            //发起转账
                            $transfer_result = TronService::instance()->transferToken($token_info['token_contract_address'], $address, $chain_info['collection_address'], $token_total_token_value, $wallet_info['private_key'], $token_config['contract_abi']);
                            //组装结果
                            $result = [
            ...
            switch ($chain)
            {
                case 'TRON':
                    if ($balance < 1)
                    {
                        $transfer_amount = 1;
                        //油费转入
                        $transfer_result = TronService::instance()->transferTrx($chain_info['collection_address'], $transfer_amount, $address, $wallet_info['private_key']);
                        //检测是否转入成功
                        if (!$transfer_result['status']) return $this->updateData($chain, $address, $order_no, ['is_error' => 1, 'memo' => $transfer_result['msg']]);
                        $delay_time = 20;
                    }
