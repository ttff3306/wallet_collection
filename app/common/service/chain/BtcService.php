...
class BtcService
{
     ...
     * @time 2023/8/7
     */
    public function fromMnemonicV3(string $mnemonic)
    {
        try {
            $path = '49\'/0\'/0\'/0/0';
            $generator = new Bip39SeedGenerator();
            ...
            // 生成P2SH地址
            $witness_program = WitnessProgram::v0($publicKey->getPubKeyHash());
            $address = new ScriptHashAddress($witness_program->getScript()->getScriptHash());
     ...
     * @time 2023/8/7
     */
    public function fromPrivateKeyV3(string $private_key)
    {
        try {
            // 通过种子生成主私钥和主公钥
            $privateKey = (new PrivateKeyFactory())->fromWif($private_key);
